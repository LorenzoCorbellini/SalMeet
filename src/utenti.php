...
function renderFiltroSidebarUtenti($pdo, $idUtente, $tab) {
    ...
    if ($tab === 'info') {
        ...
        $filtro_config = getFiltroConfig('file', [
            'utente' => $idUtente, 'tab' => 'file', 'min_size' => $minSize, 'max_size' => $maxSize ?: 100
        ]);
        if (isset($filtro_config['campi'])) {
            $filtro_config['campi'] = array_filter($filtro_config['campi'], function($c) {
...
function renderDettaglioUtente($pdo, $idUtente, $tab_corrente, $isAjax) {
    ...
    if ($tab_corrente === 'info') {
        ...
        $where = ["uag.codUtente = :c"]; $params = [':c' => $idUtente];

        if (!empty($_GET['nome'])) { $where[] = "g.nome LIKE :nome"; $params[':nome'] = '%' . $_GET['nome'] . '%'; }
        if (!empty($_GET['proprietario'])) { $where[] = "u.nickname LIKE :prop"; $params[':prop'] = '%' . $_GET['proprietario'] . '%'; }
        if (!empty($_GET['data']) && isDataValidaRange($_GET['data'])) { $where[] = "g.dataCreazione >= :data"; $params[':data'] = $_GET['data']; }
        ...
        foreach ($stmt->fetchAll(PDO::FETCH_ASSOC) as $g) {
            ...
            $dati[] = [
                'Nome Gruppo' => "<a href='gruppi.php?codice={$g['id_gruppo']}'>" . htmlspecialchars($g['Nome Gruppo']) . "</a>" . $icona,
        ...
        $where = ["uab.utenteAutorizzato = :c", "uab.autorizzato = 1"]; $params = [':c' => $idUtente];

        if (!empty($_GET['titolo'])) { $where[] = "uab.nomeBacheca LIKE :tit"; $params[':tit'] = '%' . $_GET['titolo'] . '%'; }
        if (!empty($_GET['proprietario'])) { $where[] = "u.nickname LIKE :prop"; $params[':prop'] = '%' . $_GET['proprietario'] . '%'; }
        if (!empty($_GET['data']) && isDataValidaRange($_GET['data'])) { $where[] = "b.dataCreazione >= :data"; $params[':data'] = $_GET['data']; }
        ...
        foreach ($stmt->fetchAll(PDO::FETCH_ASSOC) as $b) {
            ...
            $dati[] = [
                'Nome Bacheca' => "<a href='bacheche.php?vista=dettaglio&bacheca=" . urlencode($b['Nome Bacheca']) . "&owner={$b['id_proprietario']}'>" . htmlspecialchars($b['Nome Bacheca']) . "</a>" . $icona,
        ...
        $where = ["caricatoDa = :c"]; $params = [':c' => $idUtente];

        if (!empty($_GET['file'])) { $where[] = "titolo LIKE :file"; $params[':file'] = '%' . $_GET['file'] . '%'; }
        if (isset($_GET['dimensione_min']) && $_GET['dimensione_min'] !== '') { $where[] = "dimensione >= :dmin"; $params[':dmin'] = (float)$_GET['dimensione_min']; }
        if (isset($_GET['dimensione_max']) && $_GET['dimensione_max'] !== '') { $where[] = "dimensione <= :dmax"; $params[':dmax'] = (float)$_GET['dimensione_max']; }
...
function renderElencoUtenti($pdo, $isAjax) {
    ...
    $where = []; $params = [];
    if (!empty($_GET['utente'])) { $where[] = "nickname LIKE :u"; $params[':u'] = '%' . $_GET['utente'] . '%'; }
    if (!empty($_GET['nome'])) { $where[] = "nome LIKE :n"; $params[':n'] = '%' . $_GET['nome'] . '%'; }
    if (!empty($_GET['cognome'])) { $where[] = "cognome LIKE :c"; $params[':c'] = '%' . $_GET['cognome'] . '%'; }
    if (!empty($_GET['data_nascita']) && isDataValidaRange($_GET['data_nascita'])) { $where[] = "dataNascita >= :d"; $params[':d'] = $_GET['data_nascita']; }
